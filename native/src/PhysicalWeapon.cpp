...
// The reference port uses p = p0 + v*t - up*(2.1*t*t), i.e. 4.2 m/s^2 acceleration.
...
bool IsTwoHandedWeaponTypeInternal(int weaponType) {
    // SA equivalents of the reference Quest long-gun list. Compact SMGs and
...
void StartDropLocked(int hand, int slot, double now) {
    ...
    // Keep one flying record per source hand; replacing this record makes
...
bool BuildSupportAnchorLocked(int primaryHand, TrackingPose* out) {
    ...
    // Model-bound support contract: model RIGHT is the barrel,
    ...
    // RIGHT is the one saved/master profile. Match the reference port's derived LEFT
    ...
    // The Quest wrist contract is style-dependent. Build the
...
void Init() {
    ...
    ResetRuntimeLocked();
    LOGI("[physwpn] initialised (dual-hand mode)");
}
...
void Update(bool interactionsBlocked, bool showHolsterMarkersWhileBlocked,
    ...
    if (!interactionsBlocked) {
        ...
        // weapon -- the current Quest hand-off contract.
        for (int primary = 0; primary < kHandCount; ++primary) {
        ...
        for (int receiver = 0; receiver < kHandCount; ++receiver) {
            ...
            // Accept either the final rendered origin (reference behavior) or the
    ...
    if (gameplayMarkers || holsterCalibrationMarkers ||
        ...
        // The model-bound foregrip uses the same marker
